...
void StepperRMT::_execMove(const MotorMove& m) {
    ...
    const long   absTotal = (m.steps < 0) ? -m.steps : m.steps;
    ...
    for (long done = 0; done < absTotal && !_stop.load(); ) {
        ...
        // ── Brzina prema S-krivoj profila (smoothstep) ────────────────
        // Smoothstep: f(t) = 3t² - 2t³  →  prvi i drugi izvod = 0 na rubovima
        // = nema trzaja (jerk=0) na početku ubrzanja i kraju usporenja
        // = motor glatko kreće i staje bez šokova → manje vibracija
        float speed;
        bool  inRamp;
